...
Scenario: Define variables
    ...
    * def tableData =
    ...
    | Alice | 25  |
    """
    * def yamlData = read('data.yaml')
    * def csvData = read('data.csv')
```
...
Scenario: Demonstrating actions
    ...
    * remove json.age
    * print json
    * replace json.name = 'Doe'
    * def script = 'function add(a, b) { return a + b }'
    * eval script
    * def sum = add(5, 10)
    * print "Sum is", sum
```
...
Scenario: Validate JSON response
    ...
    And match response.email contains '@gmail.com'
    And match response.address.city != 'London'
```
...
---
This README provides a deep dive into Karate's powerful features with detailed explanations and examples. Happy testing! 🚀
